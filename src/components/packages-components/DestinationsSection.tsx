...
const DestinationsSection: React.FC<DestinationsSectionProps> = ({ destinations }) => {
  ...
  return (
    <div className="bg-white rounded-lg sm:rounded-xl lg:rounded-2xl shadow-md sm:shadow-lg p-4 sm:p-5 lg:p-6">
  <h2 className="text-xl sm:text-2xl lg:text-3xl font-bold text-gray-900 mb-4 sm:mb-5 lg:mb-6">
  ...
  <div className="space-y-4 sm:space-y-5 lg:space-y-6">
    {destinations.map((destination) => (
      <div
        ...
        className="border border-gray-200 rounded-lg sm:rounded-xl overflow-hidden hover:shadow-sm sm:hover:shadow-md transition-shadow"
      >
        {/* Destination Header */}
        <button
          onClick={() => toggleDestination(destination.destinationId)}
          className="w-full p-3 sm:p-4 lg:p-6 text-left bg-gradient-to-r from-gray-50 to-white hover:from-gray-100 transition-colors"
        >
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3 sm:gap-4">
            <div className="flex-1 min-w-0">
              ...
              <p className="text-gray-600 text-sm sm:text-base mb-2 line-clamp-2 sm:line-clamp-3">
              ...
              <div className="flex flex-wrap gap-1.5 sm:gap-2">
                ...
                <span className="px-2 py-1 bg-green-100 text-green-800 rounded-full text-xs sm:text-sm truncate max-w-[120px] sm:max-w-none">
                  {destination.location}
                </span>
                <span className="md:hidden px-2 py-1 bg-purple-100 text-purple-800 rounded-full text-xs">
        ...
        {expandedDestination === destination.destinationId && (
          <div className="p-3 sm:p-4 lg:p-6 border-t border-gray-200 animate-fadeIn">
            {/* Destination Images */}
            {destination.images.length > 0 && (
              <div className="mb-4 sm:mb-5 lg:mb-6">
                <h4 className="text-base sm:text-lg font-semibold text-gray-800 mb-2 sm:mb-3">Gallery</h4>
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-2 sm:gap-3">
                  {destination.images.map((image) => (
                    <div
                      key={image.imageId}
                      className="relative aspect-square rounded-lg overflow-hidden bg-gray-200 group cursor-pointer"
                      // onClick={() => toggleDestination(image.imageUrl, image.imageDescription)}
                    >
                      <Image
                        src={image.imageUrl}
                ...
                <div className="space-y-3 sm:space-y-4">
                  {destination.activities.map((activity) => (
                    <div
                      ...
                      className="p-3 sm:p-4 border border-gray-200 rounded-lg sm:rounded-xl hover:border-purple-300 transition-colors"
                    >
                      <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-3 mb-2 sm:mb-3">
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex flex-wrap gap-1.5 sm:gap-2">
                            <span className="sm:hidden px-2 py-1 bg-purple-100 text-purple-800 rounded text-xs">
                              {activity.activitiesCategory}
                            </span>
