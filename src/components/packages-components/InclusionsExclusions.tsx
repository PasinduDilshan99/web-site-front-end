...
const InclusionsExclusions: React.FC<InclusionsExclusionsProps> = ({ packageData }) => {
  ...
  return (
    <div className="bg-white rounded-xl sm:rounded-2xl shadow-lg p-4 sm:p-6 border border-sky-100">
      ...
      {/* Content */}
      <div>
        <h3 className="text-lg sm:text-xl font-semibold text-sky-800 mb-3 sm:mb-4">{getTabTitle()}</h3>
        <div className="space-y-2 sm:space-y-3">
          {getActiveItems().map((item) => (
            ...
            >
              {activeTab === 'inclusions' ? (
                <svg className="w-4 h-4 sm:w-5 sm:h-5 text-teal-500 flex-shrink-0 mt-0.5" fill="currentColor" viewBox="0 0 20 20">
