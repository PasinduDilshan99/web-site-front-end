...
const PopularTags: React.FC<PopularTagsProps> = ({ tagList, loadingTags, onTagClick }) => {
  return (
    <div className="bg-white rounded-3xl shadow-xl p-6 border border-teal-200">
      <h3 className="text-xl font-bold text-teal-800 mb-6">Popular Tags</h3>
      {loadingTags ? (
        ...
        <div className="flex flex-wrap gap-2">
          {tagList.map((tag) => (
            <button
              ...
              className="px-3 py-1.5 bg-gradient-to-r from-teal-50 to-blue-50 text-teal-700 rounded-full text-sm hover:from-teal-100 hover:to-blue-100 transition-colors cursor-pointer border border-teal-200"
            >
              #{tag.name}
            </button>
          ))}
