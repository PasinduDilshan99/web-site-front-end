    ...
    const fetchHeroData = async () => {
      try {
        ...
        const response = await fetch(
          "http://localhost:8080/felicita/v0/api/hero-section/destination",
          {
            method: "GET",
            headers: {
        ...
        if (apiResponse.code === 200 && apiResponse.data) {
          ...
          setError(apiResponse.message || "Failed to fetch destinations content");
        }
      } catch (err) {
        console.error("Error fetching destinations hero data:", err);
        setError(
          err instanceof Error ? err.message : "Failed to load destinations content"
        );
      } finally {
        setLoading(false);
      }
