import React from "react";

const UserProfileActivityReviewsLoading = () => {
  return (
    <div className="flex-1 p-4 md:p-6 lg:p-8 bg-gradient-to-br from-slate-900 via-gray-900 to-teal-950 min-h-screen">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        {/* Simple loading header */}
        <div className="flex justify-center mb-8">
          <div className="flex items-center space-x-3 px-4 py-2 bg-gray-900/50 backdrop-blur-sm rounded-full border border-teal-500/30">
            <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-teal-400"></div>
            <span className="text-teal-300 text-sm">
              Loading activity reviews...
            </span>
          </div>
        </div>

        {/* Header Section Skeleton */}
        <div className="mb-8 md:mb-12">
          <div className="h-8 md:h-9 lg:h-10 bg-gradient-to-r from-gray-700 to-teal-800/50 rounded-lg w-48 md:w-56 lg:w-64 mb-3 animate-pulse"></div>
          <div className="h-4 md:h-5 lg:h-6 bg-gradient-to-r from-gray-700 to-cyan-800/40 rounded w-64 md:w-72 lg:w-80 animate-pulse"></div>

          {/* Stats Summary Skeleton */}
          <div className="mt-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {[...Array(2)].map((_, i) => (
              <div
                key={i}
                className="bg-gradient-to-br from-gray-800/80 to-teal-900/30 rounded-xl border border-teal-500/20 p-4"
              >
                <div className="h-3 bg-gradient-to-r from-gray-700 to-cyan-800/40 rounded w-20 mb-2 animate-pulse"></div>
                <div className="h-6 bg-gradient-to-r from-gray-700 to-teal-800/50 rounded w-12 animate-pulse"></div>
              </div>
            ))}
          </div>
        </div>

        {/* Activity Reviews List Skeleton */}
        <div className="space-y-6 lg:space-y-8">
          {[...Array(2)].map((_, reviewIndex) => (
            <div
              key={reviewIndex}
              className="bg-gradient-to-br from-gray-800/80 to-teal-900/30 rounded-2xl border border-teal-500/20 overflow-hidden"
            >
              <div className="p-6 md:p-8">
                {/* Review Header Skeleton */}
                <div className="flex flex-col sm:flex-row justify-between items-start gap-4 mb-6">
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-2">
                      <div className="w-10 h-10 bg-gradient-to-br from-gray-700 to-teal-800/50 rounded-full animate-pulse"></div>
                      <div>
                        <div className="h-4 bg-gradient-to-r from-gray-700 to-teal-800/50 rounded w-32 mb-1 animate-pulse"></div>
                        <div className="h-3 bg-gradient-to-r from-gray-700 to-cyan-800/40 rounded w-24 animate-pulse"></div>
                      </div>
                    </div>
                    <div className="flex items-center space-x-1">
                      {[...Array(5)].map((_, i) => (
                        <div
                          key={i}
                          className="w-5 h-5 bg-gradient-to-br from-gray-700 to-teal-800/50 rounded"
                        ></div>
                      ))}
                    </div>
                  </div>
                  <div className="flex flex-col sm:items-end gap-2">
                    <div className="w-24 h-6 bg-gradient-to-r from-teal-800 to-teal-800/50 rounded-full"></div>
                    <div className="w-20 h-5 bg-gradient-to-r from-gray-700 to-gray-800/50 rounded-full"></div>
                  </div>
                </div>

                {/* Review Text Skeleton */}
                <div className="mb-6 space-y-2">
                  <div className="h-3 bg-gradient-to-r from-gray-700 to-cyan-800/40 rounded w-full animate-pulse"></div>
                  <div className="h-3 bg-gradient-to-r from-gray-700 to-cyan-800/40 rounded w-5/6 animate-pulse"></div>
                  <div className="h-3 bg-gradient-to-r from-gray-700 to-cyan-800/40 rounded w-4/6 animate-pulse"></div>
                </div>

                {/* Reactions Section Skeleton */}
                <div className="mb-6 p-4 bg-gradient-to-br from-gray-800/50 to-teal-900/20 rounded-xl border border-teal-500/20">
                  <div className="flex flex-col sm:flex-row sm:items-center gap-3">
                    <div className="h-3 bg-gradient-to-r from-gray-700 to-teal-800/50 rounded w-16 animate-pulse"></div>
                    <div className="flex flex-wrap gap-2">
                      {[...Array(3)].map((_, i) => (
                        <div
                          key={i}
                          className="w-16 h-6 bg-gradient-to-r from-gray-700 to-teal-800/50 rounded-full"
                        ></div>
                      ))}
                    </div>
                  </div>
                </div>

                {/* Images Grid Skeleton */}
                <div className="mb-6">
                  <div className="flex items-center justify-between mb-3">
                    <div className="h-4 bg-gradient-to-r from-gray-700 to-teal-800/50 rounded w-16 animate-pulse"></div>
                    <div className="h-3 bg-gradient-to-r from-gray-700 to-cyan-800/40 rounded w-16 animate-pulse"></div>
                  </div>
                  <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-2 sm:gap-3">
                    {[...Array(4)].map((_, i) => (
                      <div
                        key={i}
                        className="relative aspect-square bg-gradient-to-br from-gray-700 to-teal-800/50 rounded-lg animate-pulse"
                      ></div>
                    ))}
                  </div>
                </div>

                {/* Comments Section Skeleton */}
                <div className="border-t border-teal-500/20 pt-6">
                  <div className="flex items-center justify-between mb-4">
                    <div className="h-4 bg-gradient-to-r from-gray-700 to-teal-800/50 rounded w-24 animate-pulse"></div>
                    <div className="h-3 bg-gradient-to-r from-gray-700 to-cyan-800/40 rounded w-20 animate-pulse"></div>
                  </div>
                  <div className="space-y-4">
                    {[...Array(2)].map((_, i) => (
                      <div
                        key={i}
                        className="bg-gradient-to-br from-gray-800/50 to-teal-900/20 rounded-xl p-4 border border-teal-500/20"
                      >
                        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-2">
                          <div className="flex items-center gap-2">
                            <div className="w-6 h-6 bg-gradient-to-br from-gray-700 to-teal-800/50 rounded-full"></div>
                            <div className="h-3 bg-gradient-to-r from-gray-700 to-teal-800/50 rounded w-20 animate-pulse"></div>
                          </div>
                          <div className="h-2 bg-gradient-to-r from-gray-700 to-cyan-800/40 rounded w-16 animate-pulse"></div>
                        </div>
                        <div className="space-y-1">
                          <div className="h-2 bg-gradient-to-r from-gray-700 to-cyan-800/40 rounded w-full animate-pulse"></div>
                          <div className="h-2 bg-gradient-to-r from-gray-700 to-cyan-800/40 rounded w-3/4 animate-pulse"></div>
                        </div>
                      </div>
                    ))}
                  </div>
                </div>

                {/* Footer Skeleton */}
                <div className="flex flex-col sm:flex-row justify-between items-center gap-3 mt-6 pt-6 border-t border-teal-500/20">
                  <div className="flex items-center gap-2">
                    <div className="w-20 h-6 bg-gradient-to-r from-emerald-800 to-emerald-800/50 rounded-full"></div>
                    <div className="w-16 h-4 bg-gradient-to-r from-gray-700 to-cyan-800/40 rounded"></div>
                  </div>
                  <div className="flex gap-3">
                    <div className="w-20 h-6 bg-gradient-to-r from-sky-800 to-teal-800/50 rounded"></div>
                    <div className="w-16 h-6 bg-gradient-to-r from-teal-800 to-teal-800/50 rounded"></div>
                  </div>
                </div>
              </div>
            </div>
          ))}
        </div>

        {/* Load More Button Skeleton */}
        <div className="mt-8 text-center">
          <div className="h-10 bg-gradient-to-r from-sky-800 to-teal-800/50 rounded-lg w-32 mx-auto animate-pulse"></div>
        </div>
      </div>
    </div>
  );
};

export default UserProfileActivityReviewsLoading;
