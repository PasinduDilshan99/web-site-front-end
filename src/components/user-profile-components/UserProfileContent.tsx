...
export default function UserProfileContent({
      ...
      <div className="flex-1 p-4 md:p-6 lg:p-8 flex items-center justify-center min-h-[60vh]">
        <div className="max-w-md w-full mx-auto text-center">
          <div className="bg-gradient-to-br from-white to-gray-50 rounded-2xl shadow-xl border border-gray-200 p-8">
            ...
            <h3 className="text-xl font-semibold text-gray-800 mb-3">Profile Unavailable</h3>
            <p className="text-gray-600 mb-6">We couldn&apos;t load your profile information at this time.</p>
            <button
  ...
  const InfoCard = ({ children, className = "" }: { children: React.ReactNode; className?: string }) => (
    <div className={`bg-white rounded-xl shadow-sm border border-gray-200 hover:shadow-md transition-all duration-300 hover:border-sky-200 p-5 ${className}`}>
      {children}
    </div>
  );
  ...
  const InfoField = ({ label, value, className = "" }: { label: string; value: string; className?: string }) => (
    <div className={`mb-4 last:mb-0 ${className}`}>
      ...
      <p className="text-gray-800 font-medium text-sm lg:text-base leading-relaxed truncate">{value || "Not provided"}</p>
    </div>
  );
  ...
  const StatBadge = ({ label, value, color = "sky" }: { label: string; value: string; color?: "sky" | "teal" | "green" | "gray" }) => {
    const colorClasses = {
    ...
    return (
      <div className="flex flex-col items-center text-center p-4 rounded-xl border bg-white hover:shadow-sm transition-all duration-200">
        ...
        <span className={`text-lg font-semibold px-3 py-1 rounded-lg border ${colorClasses[color]}`}>
          {value}
        </span>
      </div>
    );
        ...
        <div className="mb-10 md:mb-12">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              ...
              <p className="text-gray-600 text-sm md:text-base">View and manage your personal details</p>
            </div>
          ...
          <div className="lg:col-span-2 space-y-6 lg:space-y-8">
            ...
            <div className="bg-white rounded-2xl shadow-lg border border-gray-200 p-6 md:p-8">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 gap-5 md:gap-6">
                ...
                <InfoCard>
                  <InfoField label="Date of Birth" value={userProfile.dateOfBirth || "Not provided"} />
                </InfoCard>
                
                <InfoCard>
                  <InfoField label="Gender" value={userProfile.gender || "Not specified"} />
                </InfoCard>
                
                <InfoCard>
                  <InfoField label="Religion" value={userProfile.religion || "Not specified"} />
                </InfoCard>
              </div>
            </div>
            ...
            <div className="bg-white rounded-2xl shadow-lg border border-gray-200 p-6 md:p-8">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 md:gap-6">
                <InfoCard>
                  <InfoField label="Primary Mobile" value={userProfile.mobileNumber1 || "Not provided"} />
                </InfoCard>
                
                <InfoCard>
                  <InfoField label="Secondary Mobile" value={userProfile.mobileNumber2 || "Not provided"} />
                </InfoCard>
              </div>
            </div>
            ...
            <div className="bg-white rounded-2xl shadow-lg border border-gray-200 p-6 md:p-8">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 gap-5 md:gap-6">
                ...
                <div className="sm:col-span-2 lg:col-span-1 grid grid-cols-2 gap-5 md:gap-6">
                  <InfoCard>
                    <InfoField label="City" value={userProfile.city || "Not specified"} />
                  </InfoCard>
                  
                  <InfoCard>
                    <InfoField label="District" value={userProfile.district || "Not specified"} />
                  </InfoCard>
                  
                  <InfoCard>
                    <InfoField label="Province" value={userProfile.province || "Not specified"} />
                  </InfoCard>
                  
                  <InfoCard>
                    <InfoField label="Country" value={userProfile.countryName || "Not specified"} />
                  </InfoCard>
                </div>
                
                <InfoCard className="sm:col-span-2">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    <InfoField label="Postal Code" value={userProfile.postalCode || "Not provided"} />
                    <div>
                      <label className="block text-xs font-medium text-gray-500 uppercase tracking-wider mb-2">
                        Verified Status
                      </label>
          ...
          <div className="space-y-6 lg:space-y-8">
            ...
            <div className="bg-white rounded-2xl shadow-lg border border-gray-200 p-6">
              ...
              <div className="space-y-5">
                <InfoCard>
                  <InfoField label="National ID (NIC)" value={userProfile.nic || "Not provided"} />
                </InfoCard>
                
                <InfoCard>
                  <InfoField label="Passport Number" value={userProfile.passportNumber || "Not provided"} />
                </InfoCard>
                
                <InfoCard>
                  <InfoField label="Driving License" value={userProfile.drivingLicenseNumber || "Not provided"} />
                </InfoCard>
              </div>
            </div>
            ...
            <div className="bg-gradient-to-br from-sky-50 to-white rounded-2xl shadow-lg border border-sky-200 p-6">
              ...
              <div className="space-y-6">
                <div className="text-center p-4 bg-white rounded-xl border border-gray-200">
                  ...
                  <p className="text-xs text-gray-500 mt-2">Earn more points with every booking</p>
                </div>
                
                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-gradient-to-r from-sky-50 to-teal-50 rounded-xl p-4 border border-sky-100">
                    <div className="text-lg font-semibold text-sky-700 mb-1">Available</div>
                    <div className="text-2xl font-bold text-gray-800">{userProfile.benefitsPointsCount}</div>
                  </div>
                  <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-xl p-4 border border-teal-100">
                    <div className="text-lg font-semibold text-teal-700 mb-1">Value</div>
                    <div className="text-2xl font-bold text-gray-800">LKR {(userProfile.benefitsPointsCount * 10).toLocaleString()}</div>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white rounded-2xl shadow-lg border border-gray-200 p-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                <StatBadge label="User Type" value={userProfile.userType} color="sky" />
                <StatBadge label="Account Status" value={userProfile.userStatus} color="green" />
            ...
            <div className="bg-gradient-to-br from-teal-50 to-sky-50 rounded-2xl shadow-lg border border-teal-200 p-6">
              ...
              <div className="space-y-3">
                <button className="w-full px-4 py-3 bg-white text-sky-700 rounded-lg border border-sky-200 hover:bg-sky-50 hover:border-sky-300 transition-all duration-200 text-sm font-medium flex items-center justify-center">
                  <svg className="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                  </svg>
                  Update Profile
                </button>
                <button className="w-full px-4 py-3 bg-gradient-to-r from-sky-600 to-teal-600 text-white rounded-lg hover:shadow-lg transition-all duration-200 text-sm font-medium flex items-center justify-center">
                  <svg className="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
