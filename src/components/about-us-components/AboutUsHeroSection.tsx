  ...
  useEffect(() => {
    const fetchHeroData = async () => {
      try {
        ...
        const response = await fetch('http://localhost:8080/felicita/v0/api/hero-section/about-us', {
          method: 'GET',
          headers: {
        ...
        if (apiResponse.code === 200 && apiResponse.data) {
          ...
          setError(apiResponse.message || "Failed to fetch about us content");
        }
      } catch (err) {
        console.error("Error fetching about us hero data:", err);
        setError(err instanceof Error ? err.message : "Failed to load about us content");
      } finally {
        setLoading(false);
      }
    };
