...
const ContactHighlights = () => {
  ...
  const getActionButton = (method: ContactMethod) => {
    const baseClass =
      "w-full cursor-pointer px-4 py-2.5 text-white rounded-lg transition-all duration-300 text-sm font-semibold flex items-center justify-center gap-2 shadow-md hover:shadow-lg active:scale-95";
    ...
    switch (method.action) {
      case "call":
        return (
          <button
            ...
            className={`${baseClass} bg-gradient-to-r from-blue-600 to-cyan-500 hover:from-blue-700 hover:to-cyan-600`}
          >
      ...
      case "whatsapp":
        return (
          <button
            onClick={() => handleContactAction(method)}
            className={`${baseClass} bg-gradient-to-r from-green-500 to-teal-400 hover:from-green-600 hover:to-teal-500`}
          >
      ...
      case "email":
        return (
          <button
            onClick={() => handleContactAction(method)}
            className={`${baseClass} bg-gradient-to-r from-cyan-500 to-blue-500 hover:from-cyan-600 hover:to-blue-600`}
          >
            <svg className="w-4 h-4 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 8l7.89 4.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
            </svg>
      ...
      case "location":
        return (
          <button
            onClick={() => handleContactAction(method)}
            className={`${baseClass} bg-gradient-to-r from-blue-500 to-teal-400 hover:from-blue-600 hover:to-teal-500`}
          >
            <svg className="w-4 h-4 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      case "emergency":
        return (
          <button
            onClick={() => handleContactAction(method)}
            className={`${baseClass} bg-gradient-to-r from-red-500 to-red-600 hover:from-red-600 hover:to-red-700 animate-pulse`}
          >
            <svg className="w-4 h-4 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M18.364 5.636l-3.536 3.536m0 5.656l3.536 3.536M9.172 9.172L5.636 5.636m3.536 9.192l-3.536 3.536M21 12a9 9 0 11-18 0 9 9 0 0118 0zm-5 0a4 4 0 11-8 0 4 4 0 018 0z" />
            </svg>
      ...
      case "hours":
        return (
          <button
            onClick={() => handleContactAction(method)}
            className={`${baseClass} bg-gradient-to-r from-teal-500 to-emerald-400 hover:from-teal-600 hover:to-emerald-500`}
          >
            <svg className="w-4 h-4 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>
      ...
      default:
        return (
          <button
            onClick={() => handleContactAction(method)}
            className={`${baseClass} bg-gradient-to-r from-gray-500 to-gray-600 hover:from-gray-600 hover:to-gray-700`}
          >
            Contact
          </button>
        );
    ...
    return (
      <div className="py-12 px-4 md:px-8 bg-gradient-to-b from-blue-50 to-cyan-50">
        <div className="container mx-auto text-center">
          <div className="text-xl sm:text-2xl font-bold text-red-600 mb-4">
  ...
  const emergencyContact = contactMethods.find((m) => m.action === "emergency");
  const mainPhoneContact = contactMethods.find((m) => m.action === "call");
  const whatsappContact = contactMethods.find((m) => m.action === "whatsapp");

  return (
    <div className="py-10 px-4 sm:px-6 md:px-8 bg-gradient-to-b from-blue-50 via-cyan-50 to-teal-50">
      <div className="container mx-auto max-w-6xl">
        ...
        <div className="text-center mb-8 sm:mb-10 md:mb-12">
          ...
          <div className="w-20 sm:w-24 h-1 bg-gradient-to-r from-blue-600 to-teal-500 mx-auto mt-4 sm:mt-6 rounded-full" />
        </div>

        {/* Contact Cards Grid — 1 col mobile, 2 col sm, 3 col lg */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-5 md:gap-6 mb-8 sm:mb-10 md:mb-12">
          {contactMethods.map((method) => (
            <div
              key={method.id}
              className={`relative flex flex-col rounded-xl p-5 transition-all duration-300 hover:shadow-xl hover:-translate-y-0.5 ${
                method.highlight
            ...
            >
              ...
              {method.highlight && (
                <div className="absolute -top-3 left-1/2 -translate-x-1/2">
                  <span className="px-3 py-1 bg-gradient-to-r from-red-500 to-red-600 text-white text-xs font-bold rounded-full animate-pulse shadow-md whitespace-nowrap">
                    24/7 Available
                  </span>
                </div>
              )}
              ...
              <div className="flex items-start gap-4 mb-4">
                {/* Icon */}
                <div
                  className={`shrink-0 inline-flex p-3 rounded-xl ${
                    method.highlight
        ...
        {/* Banner */}
        <div className="bg-gradient-to-r from-blue-600 via-cyan-500 to-teal-500 rounded-2xl p-5 sm:p-6 md:p-8 text-white shadow-xl">
          <div className="flex flex-col gap-5 sm:gap-6 md:flex-row md:items-center md:justify-between">
            ...
            <div className="flex flex-col xs:flex-row gap-3 sm:gap-4 shrink-0">
              {mainPhoneContact && (
                <a
                  href={mainPhoneContact.link || "#"}
                  className="flex items-center justify-center gap-2 px-5 py-3 bg-white text-blue-600 font-bold rounded-lg hover:bg-blue-50 transition-all duration-300 shadow-lg hover:shadow-xl text-sm sm:text-base active:scale-95"
                >
              ...
              {whatsappContact && (
                <a
                  ...
                  className="flex items-center justify-center gap-2 px-5 py-3 bg-gradient-to-r from-green-500 to-teal-400 text-white font-bold rounded-lg hover:from-green-600 hover:to-teal-500 transition-all duration-300 shadow-lg hover:shadow-xl text-sm sm:text-base active:scale-95"
                >
