...
interface RestaurantFilterSectionProps {
  ...
  onFilterChange: (filterName: keyof RestaurantFilters, value: any) => void;
  onResetFilters: () => void;
  locations: string[];
  cuisineTypes: string[];
  amenities: string[];
...
const RestaurantFilterSection: React.FC<RestaurantFilterSectionProps> = ({
  ...
  return (
    <div className="bg-gradient-to-r from-emerald-50 to-amber-50 rounded-2xl p-6 md:p-8 mb-8 border-2 border-emerald-200 shadow-lg">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
        <h2 className="text-3xl font-bold bg-gradient-to-r from-emerald-600 to-amber-600 bg-clip-text text-transparent">
          Filter Restaurants
        </h2>
        <div className="flex gap-3">
          <button
            onClick={onResetFilters}
            className="px-6 py-2 bg-gradient-to-r from-emerald-600 to-amber-600 text-white rounded-lg hover:from-emerald-700 hover:to-amber-700 transition-all duration-300 text-sm font-semibold shadow-md hover:shadow-lg"
          >
            Reset Filters
          </button>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
        ...
        <div className="space-y-2">
          ...
          <input
            ...
            className="w-full px-4 py-2 border-2 border-emerald-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent bg-white text-gray-900 placeholder-gray-500 transition-all"
          />
        </div>
        ...
        <div className="space-y-2">
          ...
          <input
            ...
            className="w-full h-3 bg-gradient-to-r from-emerald-300 to-amber-300 rounded-lg appearance-none cursor-pointer accent-emerald-600"
          />
        </div>
        ...
        <div className="space-y-2">
          ...
          <select
            ...
            className="w-full px-4 py-2 border-2 border-emerald-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent bg-white text-gray-900 font-medium transition-all"
          >
      ...
      <div className={`relative ${showAdvancedFilters ? "mt-6" : "mb-6"}`}>
        ...
        <div className="relative flex justify-center">
          <button
            ...
            className="inline-flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-emerald-500 to-amber-500 text-white rounded-full text-sm font-semibold shadow-md hover:shadow-lg transition-all duration-300 hover:scale-105"
          >
            {showAdvancedFilters ? (
              <>
...
const ActiveFiltersSummary: React.FC<{
  ...
  const activeFilters = [
    ...
    filters.cuisineType && {
      name: "cuisineType",
      label: `Cuisine: ${filters.cuisineType}`,
      value: filters.cuisineType,
    },
    filters.location && {
      name: "location",
  ...
  const removeFilter = (filterName: string) => {
    ...
    onFilterChange(filterName as keyof RestaurantFilters, resetValues[filterName]);
  };
    ...
    <div className="border-t-2 border-emerald-200 pt-4 mt-4">
      ...
      <div className="flex flex-wrap gap-2">
        {activeFilters.map((filter: any) => (
          <span
            key={filter.name}
            className="inline-flex items-center gap-1 px-3 py-1 bg-gradient-to-r from-emerald-100 to-amber-100 text-emerald-800 rounded-full text-xs font-medium border border-emerald-200 transition-all duration-200 hover:shadow-md"
          >
            {filter.label}
            <button
