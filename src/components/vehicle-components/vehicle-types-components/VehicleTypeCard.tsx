...
const VehicleTypeCard: React.FC<VehicleTypeCardProps> = ({ vehicleType }) => {
  ...
  const handleViewVehicles = () => {
    router.push(
      `${VEHICLE_SPECIFICATION_DETAILS_PATH}?bodyType=${encodeURIComponent(vehicleType.name)}`,
    );
  };

  const handleViewDetails = () => {
    router.push(
      `${VEHICLE_TYPE_DETAILS_PATH}/${vehicleType.vehicleTypeId}?name=${encodeURIComponent(vehicleType.name)}`,
    );
  };
      ...
      <div className="p-3 sm:p-4 md:p-6">
        ...
        <div className="flex gap-2">
          <button
            ...
            className="cursor-pointer flex-1 bg-gradient-to-r from-teal-600 to-teal-700 text-white py-2 rounded-lg font-semibold hover:from-teal-700 hover:to-teal-800 transition-all duration-300 text-xs sm:text-sm shadow-md hover:shadow-lg"
          >
            View Vehicles
          </button>
          <button
            onClick={handleViewDetails}
            className="cursor-pointer flex-1 bg-gradient-to-r from-cyan-600 to-cyan-700 text-white py-2 rounded-lg font-semibold hover:from-cyan-700 hover:to-cyan-800 transition-all duration-300 text-xs sm:text-sm shadow-md hover:shadow-lg"
          >
            More Details
          </button>
        </div>
