...
interface VehicleImageSlideshowProps {
  images: VehicleImage[];
  vehicleName: string;
}

const VehicleImageSlideshow: React.FC<VehicleImageSlideshowProps> = ({
  images,
  vehicleName,
}) => {
    ...
    <div className="relative w-full h-40 sm:h-48 md:h-56 lg:h-64 group rounded-t-xl sm:rounded-t-2xl overflow-hidden">
      ...
      {validImages.map((image, idx) => (
        ...
        >
          ...
          <img
            src={image.imageUrl}
            alt={`${vehicleName} - ${image.description || `Image ${idx + 1}`}`}
...
export default VehicleImageSlideshow;