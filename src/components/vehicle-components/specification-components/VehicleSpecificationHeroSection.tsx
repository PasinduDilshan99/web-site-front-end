// components/vehicles-components/VehicleSpecificationHeroSection.tsx
"use client";
import Link from "next/link";
import React, { useState, useEffect } from "react";
import { HeroSectionService } from "@/services/heroSectionService";
import { VehicleSpecificationHeroData } from "@/types/hero-section-types";
import HeroSectionLoading from "@/components/loading-components/HeroSectionLoading";

const VehicleSpecificationHeroSection = () => {
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [heroData, setHeroData] = useState<VehicleSpecificationHeroData[]>([]);
    ...
    const fetchHeroData = async () => {
      try {
        ...
        const { data: items, error } =
          await HeroSectionService.fetchVehicleSpecificationHeroData();
        ...
        setError("Failed to load vehicle specifications content");
      } finally {
        setLoading(false);
      }
    };
  ...
  if (loading) {
    return (
      <HeroSectionLoading text="Vehicle specifications hero content loading..." />
    );
  }
          ...
          <div className="max-w-5xl text-white mx-auto text-center">
            <div className="mb-8">
              ...
              <h1 className="text-3xl md:text-4xl lg:text-5xl font-bold mb-4 leading-tight tracking-tight">
                {currentSlideData.title || "Vehicle Specifications & Details"}
              </h1>
