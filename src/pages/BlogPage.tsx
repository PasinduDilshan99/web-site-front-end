...
const BlogPage: React.FC = () => {
  ...
  const getApiUrl = (): string => {
    if (writerParam) {
      return `http://localhost:8080/felicita/v0/api/blog/writer/${encodeURIComponent(
        writerParam
      )}`;
    } else if (searchParam) {
      return `http://localhost:8080/felicita/v0/api/blog/tag/${encodeURIComponent(
        searchParam
      )}`;
    } else {
