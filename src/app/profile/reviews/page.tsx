...
import { ActivityReview, ReviewImage, ReviewReaction, ReviewComment,DestinationReview,PackageReview,TourReview } from '@/types/user-profile';
...
export default function ReviewsPage() {
  ...
  const [activeFilter, setActiveFilter] = useState<'all' | 'active' | 'inactive' | 'tour' | 'package' | 'activity' | 'destination'>('all');
  ...
  useEffect(() => {
    if (user && !user.privileges.includes(USER_PROFILE_REVIEWS_VIEW_PRIVILEGE)) {
      router.push("/profile");
    }
  }, [user, router]);
  ...
  const loadAllReviews = async () => {
    try {
      ...
      const [tourReviewsResponse, activityReviewsResponse, destinationReviewsResponse, packageReviewsResponse] = await Promise.all([
      ...
      if (tourReviewsResponse?.data) {
        tourReviews.push(...tourReviewsResponse.data.map((review: TourReview) => ({
      ...
      if (activityReviewsResponse?.data) {
        activityReviews.push(...activityReviewsResponse.data.map((review: ActivityReview) => ({
          reviewId: review.reviewId,
          reviewer: {
      ...
      if (destinationReviewsResponse?.data) {
        destinationReviews.push(...destinationReviewsResponse.data.map((review: DestinationReview) => ({
          reviewId: review.reviewId,
          reviewer: {
      ...
      if (packageReviewsResponse?.data) {
        packageReviews.push(...packageReviewsResponse.data.map((review: PackageReview) => ({
          reviewId: review.reviewId,
          reviewer: {
            id: review.createdBy,
      ...
      setReviewCategories(prev => prev.map(category => {
  ...
  const filteredReviews = allReviews.filter(review => {
    ...
    if (activeFilter === 'destination') return review.reviewType === 'DESTINATION';
    return true;
  });

  // Helper functions to get counts
  const getTourReviewsCount = () => allReviews.filter(r => r.reviewType === 'TOUR').length;
  const getActivityReviewsCount = () => allReviews.filter(r => r.reviewType === 'ACTIVITY').length;
  const getDestinationReviewsCount = () => allReviews.filter(r => r.reviewType === 'DESTINATION').length;
  const getPackageReviewsCount = () => allReviews.filter(r => r.reviewType === 'PACKAGE').length;
  const getActiveReviewsCount = () => allReviews.filter(r => r.reviewStatus === 'ACTIVE').length;
  const getInactiveReviewsCount = () => allReviews.filter(r => r.reviewStatus !== 'ACTIVE').length;
          ...
          <div className="mb-4 flex-1">
            ...
            <div className="flex flex-wrap gap-2 mb-3">
              {review.tour && (
                <span className="inline-flex items-center px-2.5 py-1 rounded-full text-xs bg-gradient-to-r from-sky-50 to-sky-100 text-sky-800 border border-sky-200 font-medium">
                  <span className="mr-1">🚌</span>
                  <span className="truncate max-w-[120px]">{review.tour.name}</span>
                </span>
              )}
              {review.activity && (
                <span className="inline-flex items-center px-2.5 py-1 rounded-full text-xs bg-gradient-to-r from-teal-50 to-teal-100 text-teal-800 border border-teal-200 font-medium">
                  <span className="mr-1">🎯</span>
                  <span className="truncate max-w-[120px]">{review.activity.name}</span>
                </span>
              )}
              {review.destination && (
                <span className="inline-flex items-center px-2.5 py-1 rounded-full text-xs bg-gradient-to-r from-cyan-50 to-cyan-100 text-cyan-800 border border-cyan-200 font-medium">
                  <span className="mr-1">🏝️</span>
                  <span className="truncate max-w-[120px]">{review.destination.name}</span>
                </span>
              )}
              {review.packageInfo && (
                <span className="inline-flex items-center px-2.5 py-1 rounded-full text-xs bg-gradient-to-r from-blue-50 to-indigo-100 text-blue-800 border border-blue-200 font-medium">
                  <span className="mr-1">📦</span>
                  <span className="truncate max-w-[120px]">{review.packageInfo.name}</span>
                </span>
              )}
              ...
              <div className="flex gap-2 overflow-x-auto pb-2 mb-3 scrollbar-thin scrollbar-thumb-sky-300 scrollbar-track-sky-100">
                {review.images.slice(0, 3).map((image) => (
                  ...
                  >
                    <img
                      ...
                      loading="lazy"
                      onError={(e) => {
                        (e.target as HTMLImageElement).src = `https://via.placeholder.com/150/93c5fd/1e3a8a?text=Review+${review.reviewId}`;
                      }}
                    />
                  </div>
                ))}
                {review.images.length > 3 && (
                  <div className="flex-shrink-0 w-16 h-16 md:w-20 md:h-20 bg-gradient-to-br from-sky-100 to-teal-100 rounded-lg flex items-center justify-center border border-sky-200">
                    <span className="text-sky-700 font-bold text-sm">+{review.images.length - 3}</span>
                  </div>
                )}
              </div>
            )}
    ...
    return (
      <div className="flex-1 p-4 md:p-6 lg:p-8 bg-gradient-to-br from-sky-25 to-teal-25 min-h-screen">
        <div className="max-w-7xl mx-auto">
          ...
          {/* Categories Loading */}
          <div className="mb-12">
            <div className="h-6 md:h-8 bg-gradient-to-r from-sky-200 to-teal-200 rounded w-40 mb-6"></div>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
              {[...Array(4)].map((_, i) => (
                <div key={i} className="h-32 md:h-36 bg-gradient-to-r from-sky-100 to-teal-100 rounded-xl md:rounded-2xl"></div>
              ))}
            </div>
          </div>

          {/* Reviews Loading */}
          <div className="h-6 md:h-8 bg-gradient-to-r from-sky-200 to-teal-200 rounded w-48 mb-6"></div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 md:gap-6">
            {[...Array(8)].map((_, i) => (
              <div key={i} className="h-48 md:h-56 bg-gradient-to-r from-sky-100 to-teal-100 rounded-xl"></div>
            ))}
          </div>
        </div>
      </div>
    ...
    <div className="flex-1 p-4 md:p-6 lg:p-8 bg-gradient-to-br from-sky-25 to-teal-25 min-h-screen">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-8 md:mb-12">
          <h2 className="text-xl md:text-2xl font-semibold text-gray-800 mb-4 md:mb-6">Review Categories</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
                ...
                <div className="bg-white rounded-xl md:rounded-2xl shadow-lg border border-sky-200 p-4 md:p-6 hover:shadow-xl transition-all duration-300 h-full">
                  <div className="flex flex-col h-full">
                    <div className="flex items-center space-x-4 mb-4">
                      <div className={`w-12 h-12 md:w-16 md:h-16 bg-gradient-to-r ${type.color} rounded-xl md:rounded-2xl flex items-center justify-center text-xl md:text-2xl text-white flex-shrink-0`}>
                        {type.icon}
                      </div>
        ...
        {filteredReviews.length === 0 ? (
          <div className="bg-white rounded-xl md:rounded-2xl shadow-lg border border-sky-200 p-6 md:p-8 text-center">
            ...
            <h3 className="text-xl md:text-2xl font-semibold text-gray-800 mb-2">No Reviews Found</h3>
          ...
          <>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 md:gap-6">
              {filteredReviews.map((review) => (
                <ReviewCard key={`${review.reviewType}-${review.reviewId}`} review={review} />
              ))}
            </div>
        ...
        {allReviews.length > 0 && (
          <div className="mt-8 md:mt-12 bg-white rounded-xl md:rounded-2xl shadow-lg border border-sky-200 p-6 md:p-8">
            <h3 className="text-lg md:text-xl font-semibold text-gray-800 mb-4 md:mb-6">Review Statistics</h3>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div className="text-center p-4 bg-gradient-to-br from-sky-50 to-sky-100 rounded-lg border border-sky-200">
                <div className="text-2xl md:text-3xl font-bold text-sky-600">{allReviews.length}</div>
