...
export default function WalletPage() {
  ...
  const loadWalletData = async () => {
    try {
      setLoading(true);
      const data = await apiService.fetchContentByUrl('/wallet');
      setWalletData(data);
    } catch (error) {
        ...
        <div className="max-w-6xl mx-auto">
          <div className="animate-pulse">
            ...
            <div className="space-y-4">
              {[...Array(3)].map((_, i) => (
                <div key={i} className="h-24 bg-gradient-to-r from-amber-100 to-purple-100 rounded-lg"></div>
              ))}
            </div>
          </div>
        </div>
    ...
    <div className="flex-1 p-4 md:p-6 lg:p-8 bg-gradient-to-br from-amber-25 to-purple-25 min-h-screen">
      <div className="max-w-6xl mx-auto">
        <div className="bg-white rounded-2xl shadow-lg border border-amber-200 p-6 md:p-8">
          ...
          <p className="text-gray-600 mb-6">Your wallet details and transaction history</p>
