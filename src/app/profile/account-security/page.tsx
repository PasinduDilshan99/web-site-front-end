...
import { AccountSecurityData, EmailVerification, MobileVerification } from '@/types/account-security';
import { useState, useEffect } from 'react';

export default function AccountSecurityPage() {
  const [securityData, setSecurityData] = useState<AccountSecurityData | null>(null);
  ...
  const getVerificationStatus = (verifications: any[], type: 'primary' | 'secondary' | 'secondory') => {
    const verification = verifications.find(v => 
      v.whichEmail === type || v.whichMobile === type
    );
    return verification || { statusName: 'Not Verified', statusDescription: 'Not verified yet' };
  };
  ...
  const handleRequestMobileOtp = async (whichMobile: 'primary' | 'secondory') => {
    ...
    try {
      ...
      setActiveVerification({ type: 'mobile', which: whichMobile, step: 'verify' });
  ...
  const handleRequestEmailOtp = async (whichEmail: 'primary' | 'secondary') => {
    const emailToVerify = whichEmail === 'primary' ? securityData?.email : email;
    ...
    try {
      ...
      setActiveVerification({ type: 'email', which: whichEmail, step: 'verify' });
      ...
      showMessage('error', 'Failed to send verification email. Please try again.');
    } finally {
      setSendingOtp(false);
    }
  };
        ...
        <div className="max-w-4xl mx-auto">
          <div className="animate-pulse">
            ...
            <div className="space-y-6">
              {[...Array(4)].map((_, i) => (
                <div key={i} className="h-32 bg-gradient-to-r from-amber-100 to-purple-100 rounded-xl"></div>
              ))}
            </div>
          </div>
        </div>
      ...
      <div className="flex-1 p-4 md:p-6 lg:p-8 bg-gradient-to-br from-amber-25 to-purple-25 min-h-screen">
        <div className="max-w-4xl mx-auto">
          <div className="bg-white rounded-2xl shadow-lg border border-amber-200 p-8 text-center">
            ...
            <h3 className="text-xl font-semibold text-gray-800 mb-2">Unable to Load Security Settings</h3>
      ...
      <div className="flex-1 p-4 md:p-6 lg:p-8 bg-gradient-to-br from-amber-25 to-purple-25 min-h-screen">
        <div className="max-w-4xl mx-auto">
          <div className="bg-white rounded-2xl shadow-lg border border-amber-200 p-8 text-center">
            ...
            <h3 className="text-xl font-semibold text-gray-800 mb-2">No Security Data Found</h3>
            <p className="text-gray-600">Unable to load your account security information.</p>
          </div>
        </div>
      </div>
    );
  ...
  const primaryMobileVerification = getVerificationStatus(securityData.mobileVerifications, 'primary');
  const secondaryMobileVerification = getVerificationStatus(securityData.mobileVerifications, 'secondory');
  const primaryEmailVerification = getVerificationStatus(securityData.emailVerifications, 'primary');
  const secondaryEmailVerification = getVerificationStatus(securityData.emailVerifications, 'secondary');
    ...
    <div className="flex-1 p-4 md:p-6 lg:p-8 bg-gradient-to-br from-amber-25 to-purple-25 min-h-screen">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-600 mt-2">Manage your contact information and verification status</p>
        </div>
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-2xl shadow-xl max-w-md w-full p-6">
              <h3 className="text-xl font-bold text-gray-800 mb-4">
                Verify {activeVerification.type === 'mobile' ? 'Mobile Number' : 'Email Address'}
              </h3>
              
              {activeVerification.step === 'request' ? (
                <div className="space-y-4">
                  <p className="text-gray-600">
                    Enter your {activeVerification.type} to receive a verification code:
                  </p>
                  <input
                    type={activeVerification.type === 'mobile' ? 'tel' : 'email'}
                    value={activeVerification.type === 'mobile' ? mobileNumber : email}
                    onChange={(e) => activeVerification.type === 'mobile' ? setMobileNumber(e.target.value) : setEmail(e.target.value)}
                    placeholder={activeVerification.type === 'mobile' ? 'Enter mobile number' : 'Enter email address'}
                  ...
                  <div className="flex space-x-3">
                    ...
                    <button
                      onClick={() => activeVerification.type === 'mobile' 
                        ? handleRequestMobileOtp(activeVerification.which as 'primary' | 'secondory')
                        : handleRequestEmailOtp(activeVerification.which as 'primary' | 'secondary')
                      }
                      disabled={sendingOtp}
                ...
                <div className="space-y-4">
                  <p className="text-gray-600">
                    Enter the verification code sent to your {activeVerification.type}:
                  </p>
                  <input
                    type="text"
                    value={otpCode}
                  ...
                  <div className="flex space-x-3">
                    ...
                    <button
                      onClick={activeVerification.type === 'mobile' ? handleVerifyMobileOtp : handleVerifyEmailOtp}
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-amber-200 overflow-hidden mb-6">
          <div className="bg-gradient-to-r from-amber-500 to-purple-600 p-6 text-white">
            ...
            <p className="text-amber-100 text-sm mt-1">Secure your account with verified mobile numbers</p>
          </div>
          ...
          <div className="divide-y divide-gray-100">
            ...
            <div className="p-6">
              <div className="flex items-center justify-between">
                <div className="flex-1">
                  <h3 className="font-semibold text-gray-800 text-lg">Primary Mobile Number</h3>
                  <p className="text-gray-600 text-sm mt-1">
                    {maskMobileNumber(securityData.mobileNumber1)}
                  </p>
                  <div className="flex items-center space-x-2 mt-2">
                    <span className={`px-2 py-1 rounded-full text-xs font-semibold border ${getStatusColor(primaryMobileVerification.statusName)}`}>
                ...
                <button
                  onClick={() => setActiveVerification({ type: 'mobile', which: 'primary', step: 'request' })}
                ...
                >
                  {primaryMobileVerification.statusName === 'Verified' ? 'Re-verify' : 'Verify'}
                </button>
              </div>
            </div>
            ...
            <div className="p-6">
              <div className="flex items-center justify-between">
                <div className="flex-1">
                  <h3 className="font-semibold text-gray-800 text-lg">Secondary Mobile Number</h3>
                  <p className="text-gray-600 text-sm mt-1">
                    {securityData.mobileNumber2 ? maskMobileNumber(securityData.mobileNumber2) : 'Not set'}
                  </p>
                  {securityData.mobileNumber2 && (
                    <div className="flex items-center space-x-2 mt-2">
                      <span className={`px-2 py-1 rounded-full text-xs font-semibold border ${getStatusColor(secondaryMobileVerification.statusName)}`}>
                ...
                <button
                  onClick={() => setActiveVerification({ type: 'mobile', which: 'secondory', step: 'request' })}
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-purple-200 overflow-hidden">
          <div className="bg-gradient-to-r from-purple-500 to-amber-600 p-6 text-white">
            ...
            <p className="text-purple-100 text-sm mt-1">Keep your email addresses verified for important updates</p>
          </div>
          ...
          <div className="divide-y divide-gray-100">
            ...
            <div className="p-6">
              <div className="flex items-center justify-between">
                <div className="flex-1">
                  <h3 className="font-semibold text-gray-800 text-lg">Primary Email Address</h3>
                  <p className="text-gray-600 text-sm mt-1">
                    {securityData.email}
                  </p>
                  <div className="flex items-center space-x-2 mt-2">
                    <span className={`px-2 py-1 rounded-full text-xs font-semibold border ${getStatusColor(primaryEmailVerification.statusName)}`}>
                ...
                <button
                  onClick={() => setActiveVerification({ type: 'email', which: 'primary', step: 'request' })}
                  className="px-4 py-2 bg-amber-500 text-white rounded-lg hover:bg-amber-600 transition-colors duration-200 text-sm font-semibold"
                >
                  {primaryEmailVerification.statusName === 'Verified' ? 'Re-verify' : 'Verify'}
                </button>
              </div>
            </div>
            ...
            <div className="p-6">
              <div className="flex items-center justify-between">
                <div className="flex-1">
                  <h3 className="font-semibold text-gray-800 text-lg">Secondary Email Address</h3>
                  <p className="text-gray-600 text-sm mt-1">
                    Add a secondary email for backup and recovery
                  </p>
                  {secondaryEmailVerification.statusName !== 'Not Verified' && (
                    <div className="flex items-center space-x-2 mt-2">
                      <span className={`px-2 py-1 rounded-full text-xs font-semibold border ${getStatusColor(secondaryEmailVerification.statusName)}`}>
                ...
                <button
                  onClick={() => setActiveVerification({ type: 'email', which: 'secondary', step: 'request' })}
                  className="px-4 py-2 bg-purple-500 text-white rounded-lg hover:bg-purple-600 transition-colors duration-200 text-sm font-semibold"
                >
                  Add & Verify
                </button>
