...
import { AuthService, SecretQuestion, UpdateSecretQuestionsRequest, UserSecretQuestion } from "@/services/authService";

const SecretQuestionUpdatePage = () => {
  const router = useRouter();
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);
  const [existingQuestions, setExistingQuestions] = useState<UserSecretQuestion[]>([]);
  const [availableQuestions, setAvailableQuestions] = useState<SecretQuestion[]>([]);
  const [error, setError] = useState<string | null>(null);
  const [success, setSuccess] = useState<string | null>(null);
  
  // Form state - simple array of questions
  const [questions, setQuestions] = useState<{ questionId: number; question: string; answer: string; }[]>([]);
  ...
  const handleAddQuestion = () => {
    ...
    const selectedQuestion = availableQuestions.find(q => q.questionId === newQuestionId);
    if (!selectedQuestion) {
  ...
  const prepareUpdateRequest = (): UpdateSecretQuestionsRequest => {
    ...
    const updateQuestions = questions
      .filter(q => existingQuestions.some(eq => 
    ...
    const addQuestions = questions
      .filter(q => !existingQuestions.some(eq => eq.secretQuestionId === q.questionId))
      .map(q => ({
        question: q.questionId,
        answer: q.answer
      }));
    ...
    const removeQuestionsIds = existingQuestions
      .filter(eq => !questions.some(q => q.questionId === eq.secretQuestionId))
  ...
  const getUnusedQuestions = () => {
    return availableQuestions.filter(aq => 
      !questions.some(q => q.questionId === aq.questionId)
    );
  };
        ...
        <div className="max-w-4xl mx-auto">
          <div className="animate-pulse space-y-6">
            ...
            <div className="space-y-4">
              {[...Array(3)].map((_, i) => (
                <div key={i} className="h-32 bg-sky-100 rounded-lg"></div>
              ))}
            </div>
          </div>
        </div>
        ...
        <div className="mb-8">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
            <div>
              ...
              <p className="text-sky-700">Update your secret questions for account recovery</p>
            </div>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="bg-white/90 backdrop-blur-sm rounded-lg shadow-sm border border-sky-200 p-6 mb-6">
            <div className="flex items-center mb-4">
              ...
              <div>
                <h3 className="text-lg font-semibold text-sky-800">Security Questions</h3>
                <p className="text-sky-600 text-sm">You must have exactly 3 questions</p>
              </div>
            </div>

            <div className="space-y-4 mb-8">
              {questions.map((question, index) => (
                <div key={index} className="bg-gradient-to-r from-sky-50 to-teal-50 rounded-lg border border-sky-200 p-4">
                  <div className="flex justify-between items-start mb-3">
                    <div className="flex items-center space-x-2">
                      ...
                      <span className="text-xs font-medium bg-gradient-to-r from-sky-100 to-teal-100 text-sky-700 px-2 py-1 rounded-full border border-sky-200">
                        {existingQuestions.some(eq => eq.secretQuestionId === question.questionId) ? 'Existing' : 'New'}
                      </span>
                    </div>
                    <button
                      type="button"
                  ...
                  <h4 className="font-medium text-sky-800 mb-3">{question.question}</h4>
                  
                  <div className="space-y-2">
                    <label className="block text-sm font-medium text-sky-700">Your Answer</label>
                    <input
                      type="text"
            ...
            <div className="bg-gradient-to-r from-sky-50 to-teal-50 border border-sky-200 rounded-lg p-5">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-sky-700 mb-2">Select Question</label>
                  <select
                    value={newQuestionId}
                    onChange={(e) => setNewQuestionId(e.target.value ? Number(e.target.value) : "")}
                    className="w-full px-3 py-2 border border-sky-300 rounded-lg focus:ring-2 focus:ring-sky-500 focus:border-sky-500 outline-none text-gray-700 bg-white"
                  >
                    <option value="" className="text-gray-500">Choose a question...</option>
                    {getUnusedQuestions().map(q => (
                      <option key={q.questionId} value={q.questionId} className="text-gray-700">
                        {q.question}
                      </option>
                    ))}
                  </select>
                ...
                <div>
                  <label className="block text-sm font-medium text-sky-700 mb-2">Your Answer</label>
                  <input
                    type="text"
                    value={newAnswer}
                ...
                <div className="md:col-span-2">
                  <button
                    ...
                    disabled={questions.length >= 3 || !newQuestionId || !newAnswer.trim()}
          ...
          <div className="bg-gradient-to-r from-sky-50 to-teal-50 border border-sky-200 rounded-lg p-5 mb-6">
            <div className="flex items-start">
              ...
              <div>
                <h4 className="font-semibold text-sky-800 mb-2">Important Guidelines</h4>
          ...
          <div className="bg-white/90 backdrop-blur-sm rounded-lg shadow-sm border border-sky-200 p-6">
            <div className="flex flex-col sm:flex-row justify-end gap-4">
              ...
              <button
                ...
                className={`px-6 py-2 rounded-lg font-medium transition-all duration-300 ${saving || questions.length !== 3 
