...
interface DisplayTour {
  ...
  title: string;
  description: string;
  status: "REQUESTED" | "PENDING" | "UPCOMING" | "COMPLETED" | "CANCELLED";
  startDate: string;
  endDate: string;
  location: string;
  tourType: string;
...
export default function ToursPage() {
  ...
  const [activeFilter, setActiveFilter] = useState<
    "all" | "requested" | "pending" | "upcoming" | "completed" | "cancelled"
  >("all");
  ...
  const loadAllTours = async () => {
    try {
      ...
      if (completedResponse?.data) {
        completedTours.push(
          ...completedResponse.data.map((tour: CompletedTour) => ({
      ...
      if (cancelledResponse?.data) {
        cancelledTours.push(
          ...cancelledResponse.data.map((tour: CancelledTour) => ({
      ...
      if (requestedResponse?.data) {
        requestedTours.push(
          ...requestedResponse.data.map((tour: RequestedTour) => ({
      ...
      if (upcomingResponse?.data) {
        upcomingTours.push(
          ...upcomingResponse.data.map((tour: UpcomingTour) => ({
            ...
            images:
              tour.documents
                ?.filter((doc) => doc.documentCategory === "IMAGE")
                .map((doc) => doc.documentUrl) || [],
            bookingReference: tour.bookingReference,
            bookingId: tour.bookingId,
            bookingStatus: tour.bookingStatus,
      ...
      // Process pending tours
      const pendingTours: DisplayTour[] = [];
      if (pendingResponse?.data) {
        pendingTours.push(
          ...pendingResponse.data.map((tour: PendingTour) => ({
            id: `pending_${tour.bookingId}`,
            title: tour.tourName,
            description: tour.tourDescription,
            ...
            bookingStatus: tour.bookingStatus,
            packageName: tour.packageName,
            totalPersons: 1, // Default value
            finalAmount: tour.packageTotalPrice,
            originalData: tour,
          })),
        );
        allToursData.push(...pendingTours);
      }
      ...
      setTourCategories((prev) =>
        prev.map((category) => {
          ...
          if (category.title === "Pending Tours") {
            return { ...category, count: pendingTours.length };
          }
  ...
  const filteredTours = allTours.filter((tour) => {
    ...
    if (activeFilter === "pending") return tour.status === "PENDING";
  ...
  const TourCard = ({ tour }: { tour: DisplayTour }) => {
    const getStatusColor = (status: string) => {
      switch (status) {
        case "REQUESTED":
          return "bg-gradient-to-r from-sky-50 to-blue-50 text-sky-800 border border-sky-200";
        case "PENDING":
          return "bg-gradient-to-r from-amber-50 to-orange-50 text-amber-800 border border-amber-200";
        case "UPCOMING":
          return "bg-gradient-to-r from-teal-50 to-emerald-50 text-teal-800 border border-teal-200";
        case "COMPLETED":
          return "bg-gradient-to-r from-cyan-50 to-blue-50 text-cyan-800 border border-cyan-200";
        case "CANCELLED":
          return "bg-gradient-to-r from-rose-50 to-pink-50 text-rose-800 border border-rose-200";
        default:
        ...
        case "REQUESTED":
          ...
          return (
            <div className="mt-2 flex items-center space-x-2 text-xs">
              <span
                className={`px-2 py-1 rounded-full ${
                  requestedData.approvalStatus === "PENDING_APPROVAL"
              ...
              <span className="flex items-center space-x-1 px-2 py-1 rounded-full bg-blue-50 text-blue-700 border border-blue-200">
                <span>⏱️</span>
                <span>{requestedData.requestAge || "New"}</span>
              </span>
            </div>
          );

        case "PENDING":
          const pendingData = tour.originalData as PendingTour;
          return (
            <div className="mt-2 flex items-center space-x-2 text-xs">
          ...
          return (
            <div className="mt-2 flex items-center space-x-2 text-xs">
              ...
              {cancelledData.refundedAmount > 0 && (
                <span className="px-2 py-1 rounded-full bg-purple-50 text-purple-700 border border-purple-200">
                  LKR {cancelledData.refundedAmount.toLocaleString()}
                </span>
              )}
            </div>
          );
        ...
        <div className="flex flex-col h-full">
              ...
              <div className="flex gap-2 overflow-x-auto pb-3 mb-4 scrollbar-thin scrollbar-thumb-sky-300 scrollbar-track-sky-100">
                    ...
                    <img
                      ...
                      onError={(e) => {
                        (e.target as HTMLImageElement).src =
                          `https://via.placeholder.com/150/93c5fd/1e3a8a?text=Tour+${index + 1}`;
                      }}
                    />
                  </div>
                ))}
                {tour.images.length > 3 && (
                  <div className="flex-shrink-0 w-20 h-20 sm:w-24 sm:h-24 md:w-28 md:h-28 bg-gradient-to-br from-sky-100 to-teal-100 rounded-lg flex items-center justify-center border border-sky-200">
                    <span className="text-sky-700 font-bold text-sm sm:text-base">
                      +{tour.images.length - 3}
                    </span>
                  </div>
                )}
          ...
          <div className="pt-4 border-t border-gray-50">
            <div className="flex justify-between items-center">
              ...
              <span
                className={`px-3 py-1.5 rounded-full text-xs sm:text-sm font-medium ${getStatusColor(tour.status)}`}
              >
                <span className="mr-1.5">{getStatusIcon(tour.status)}</span>
                <span className="hidden sm:inline">
                  {tour.status.charAt(0) + tour.status.slice(1).toLowerCase()}
                </span>
  ...
  if (loading) {
    return <UserProfileToursLoading />;
  }

  return (
    <div className="flex-1 p-4 sm:p-6 md:p-8 bg-gradient-to-br from-sky-50 to-teal-50 min-h-screen">
                ...
                <div className="bg-white rounded-xl sm:rounded-2xl shadow-lg border border-gray-200 p-4 sm:p-6 hover:shadow-xl transition-all duration-300 h-full">
                  <div className="flex flex-col h-full">
                    <div className="flex items-start sm:items-center space-x-3 sm:space-x-4 mb-4">
                      <div
                        className={`w-10 h-10 sm:w-12 sm:h-12 md:w-14 md:h-14 bg-gradient-to-r ${type.color} rounded-xl sm:rounded-2xl flex items-center justify-center text-lg sm:text-xl md:text-2xl text-white flex-shrink-0`}
                      >
                        {type.icon}
                      </div>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 sm:gap-6 mb-6">
            <div className="space-y-2">
              <h2 className="text-xl sm:text-2xl font-semibold text-gray-800">
                All Tours{" "}
                <span className="text-sky-600">({filteredTours.length})</span>
              </h2>
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              >
                Pending ({tourCategories[1].count})
              </button>
              <button
              ...
              >
                Upcoming ({tourCategories[2].count})
              </button>
              <button
              ...
              >
                Completed ({tourCategories[3].count})
              </button>
              <button
              ...
              >
                Cancelled ({tourCategories[4].count})
              </button>
            </div>
          </div>
        </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-5 gap-3 sm:gap-4">
              ...
              <div className="text-center p-4 bg-gradient-to-br from-amber-50 to-orange-50 rounded-lg border border-amber-200">
                <div className="text-2xl sm:text-3xl font-bold text-amber-600">
                  {tourCategories[1].count}
                </div>
                <div className="text-xs sm:text-sm text-gray-600 mt-1">
                  Pending
                </div>
              </div>
              <div className="text-center p-4 bg-gradient-to-br from-teal-50 to-emerald-50 rounded-lg border border-teal-200">
                <div className="text-2xl sm:text-3xl font-bold text-teal-600">
                  {tourCategories[2].count}
                </div>
                <div className="text-xs sm:text-sm text-gray-600 mt-1">
                  Upcoming
                </div>
              ...
              <div className="text-center p-4 bg-gradient-to-br from-cyan-50 to-blue-50 rounded-lg border border-cyan-200">
                <div className="text-2xl sm:text-3xl font-bold text-cyan-600">
                  {tourCategories[3].count}
                </div>
                <div className="text-xs sm:text-sm text-gray-600 mt-1">
                  Completed
                </div>
              </div>
              <div className="text-center p-4 bg-gradient-to-br from-rose-50 to-pink-50 rounded-lg border border-rose-200">
                <div className="text-2xl sm:text-3xl font-bold text-rose-600">
                  {tourCategories[4].count}
                </div>
                <div className="text-xs sm:text-sm text-gray-600 mt-1">
                  Cancelled
                </div>
