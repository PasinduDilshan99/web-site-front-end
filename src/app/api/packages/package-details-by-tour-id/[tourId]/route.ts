import { GET_PACKAGE_DETAILS_BY_TOUR_ID_DATA } from "@/utils/backEndConstant";
...
export async function GET(
  ...
  const { tourId } = await context.params;
    ...
    if (!GET_PACKAGE_DETAILS_BY_TOUR_ID_DATA) {
      throw new Error("Backend URL is not defined");
    }

    const backendUrl = `${GET_PACKAGE_DETAILS_BY_TOUR_ID_DATA}/${tourId}`;
